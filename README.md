...
function build_chain_target(num_qubits)
    ...
    for gate in (QkGate_X, QkGate_SX, QkGate_RZ)
        entry = target_entry_gate(gate)
        for i in 1:num_qubits
            error = 0.8e-6 * i
            duration = 1.8e-9 * i
            add_property!(entry, [i], duration, error)
        end
    ...
    meas_entry = target_entry_measure()
    for i in 1:num_qubits
        error = 0.0
        duration = 0.0
        add_property!(meas_entry, [i], duration, error)
    end
    add_instruction!(target, meas_entry)
